...
// Force the server's local time interpretation to match the user's timezone,
// so task times entered in the app (e.g. "14:00") line up with when
// reminders actually fire. Without this, Railway's container runs in UTC
// and reminders can be off by several hours.
...
const MINI_APP_URL = process.env.MINI_APP_URL || "https://day-planner-site-seven.vercel.app";
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(chatId, "Привет! Открой мини-приложение, чтобы спланировать день. Я буду присылать напоминания о задачах сюда.", {
    reply_markup: {
      inline_keyboard: [[{ text: "📅 Открыть план дня", web_app: { url: MINI_APP_URL } }]],
    },
  });
});

bot.onText(/\/plan/, (msg) => {
  bot.sendMessage(msg.chat.id, "Твой план дня:", {
    reply_markup: {
      inline_keyboard: [[{ text: "📅 Открыть план дня", web_app: { url: MINI_APP_URL } }]],
    },
  });
